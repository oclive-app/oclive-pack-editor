...
1. **打开角色包目录**：选择含 **`pipeline.ocblueprint`** 的 v2 / Stable v4 角色包根目录（与 `ROLE_PACK_SPEC` 一致）。legacy `manifest.json` 包须先用 `oclive pack migrate-to-blueprint` 迁移。
2. **表单 / JSON**：表单编辑常用字段与七维、`plugin_backends` 六槽 + `agent`（编辑器内拆为「角色门面 meta 视图」与「运行时 settings 视图」）；JSON 模式直接改全文；切回表单前会校验 JSON 语法。
3. **校验**：编辑态构建 Stable v4（导入 v2 时保持 v2）并先走历史命名的 **`validate_blueprint_v2_json`** Tauri 基础校验；最终导出目录再按 v2 / v3 / v4 精确分派到 `oclive_validation`。浏览器开发时回退 TypeScript 子集检查。
4. **回复质量锚点**：Stable v4 运行时 SSOT 为蓝图 **`runtime_config.reply_quality_anchor`**；导入 v2 时兼容并原版本写回 `meta.reply_quality_anchor`。两者为空则沿用内核 `DEFAULT_REPLY_QUALITY_ANCHOR`。导出可另写 **`prompts/reply_quality_anchor.md`** 作为人类可读镜像，绝不作为第二运行时真源。预设见 `replyQualityAnchorPreset.ts`。
...
> 编写器当前编辑器契约支持 v2 与 Stable v4，新包默认 v4。遇到 v3 / dual-core 蓝图时，编写器会阻止编辑保存；应使用主应用或专用工具维护 v3 集成配置。
